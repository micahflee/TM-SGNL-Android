...
import org.tm.archive.attachments.MmsNotificationAttachment
...
import org.tm.archive.database.model.NotificationMmsMessageRecord
    ...
    CoroutineScope(Dispatchers.IO).launch {
      if (preUploadResults != null) { //preUploadResults not null indicates attachments exists
        ...
        val attachmentIds: List<AttachmentId> = Stream.of(preUploadResults).map<AttachmentId>(
          { obj: PreUploadResult -> obj.attachmentId }).toList()
        ...
        archiveMessageOutboxMMS(context, ArchiveConstants.ProtocolType.ARCHIVE_PARAM_PROTOCOL_SEND, recipient, message, messageId, files.toTypedArray<File?>())
        for (i in files.indices) {
          val fileNameWithType = ArchiveFileUtil.getFileNameWithType(
            files[i]!!.name,
            messageId,
            databaseAttachments[i].attachmentId.rowId,
            databaseAttachments[i].contentType,
            false
          )
          val tempFileForArchiving = FileUtils.createPlaceHolderTempFile(context, fileNameWithType)
          updateArchiveSDKToSendMMSMessage(context, tempFileForArchiving.name, true)
        }
      } else {
