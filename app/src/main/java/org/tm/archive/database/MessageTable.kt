...
import org.tm.archive.attachments.DatabaseAttachment.DisplayOrderComparator
import org.tm.archive.contactshare.Contact
import org.tm.archive.conversation.MessageStyler
import org.tm.archive.database.EarlyDeliveryReceiptCache.Receipt
import org.tm.archive.database.MentionUtil.UpdatedBodyAndMentions
import org.tm.archive.database.SignalDatabase.Companion.attachments
import org.tm.archive.database.SignalDatabase.Companion.calls
import org.tm.archive.database.SignalDatabase.Companion.distributionLists
import org.tm.archive.database.SignalDatabase.Companion.groupReceipts
import org.tm.archive.database.SignalDatabase.Companion.groups
import org.tm.archive.database.SignalDatabase.Companion.mentions
import org.tm.archive.database.SignalDatabase.Companion.messageLog
import org.tm.archive.database.SignalDatabase.Companion.messages
import org.tm.archive.database.SignalDatabase.Companion.reactions
import org.tm.archive.database.SignalDatabase.Companion.recipients
import org.tm.archive.database.SignalDatabase.Companion.storySends
import org.tm.archive.database.SignalDatabase.Companion.threads
import org.tm.archive.database.documents.Document
import org.tm.archive.database.documents.IdentityKeyMismatch
import org.tm.archive.database.documents.IdentityKeyMismatchSet
import org.tm.archive.database.documents.NetworkFailure
import org.tm.archive.database.documents.NetworkFailureSet
import org.tm.archive.database.model.GroupCallUpdateDetailsUtil
import org.tm.archive.database.model.Mention
import org.tm.archive.database.model.MessageExportStatus
import org.tm.archive.database.model.MessageId
import org.tm.archive.database.model.MessageRecord
import org.tm.archive.database.model.MmsMessageRecord
import org.tm.archive.database.model.ParentStoryId
import org.tm.archive.database.model.ParentStoryId.DirectReply
import org.tm.archive.database.model.ParentStoryId.GroupReply
import org.tm.archive.database.model.Quote
import org.tm.archive.database.model.StoryResult
import org.tm.archive.database.model.StoryType
import org.tm.archive.database.model.StoryType.Companion.fromCode
...
import org.tm.archive.database.model.databaseprotos.MessageExportState
...
import org.tm.archive.database.model.databaseprotos.SessionSwitchoverEvent
import org.tm.archive.database.model.databaseprotos.ThreadMergeEvent
import org.tm.archive.dependencies.ApplicationDependencies
import org.tm.archive.groups.GroupMigrationMembershipChange
import org.tm.archive.jobs.OptimizeMessageSearchIndexJob
...
import org.tm.archive.notifications.v2.DefaultMessageNotifier.StickyThread
import org.tm.archive.recipients.Recipient
import org.tm.archive.recipients.RecipientId
import org.tm.archive.revealable.ViewOnceExpirationInfo
import org.tm.archive.revealable.ViewOnceUtil
import org.tm.archive.sms.GroupV2UpdateMessageUtil
...
open class MessageTable(context: Context?, databaseHelper: SignalDatabase) : DatabaseTable(context, databaseHelper), MessageTypes, RecipientIdDatabaseReference, ThreadIdDatabaseReference {
  ...
  open fun markSmsStatus(id: Long, status: Int) {//*TM_SA*/ make fun open
  ...
  protected open fun updateTypeBitmask(id: Long, maskOff: Long, maskOn: Long) {//*TM_SA*/ make fun protected open
  ...
  open fun setIncomingMessagesViewed(messageIds: List<Long>): List<MarkedMessageInfo> {//*TM_SA*/ make fun open
  ...
  open fun insertCallLog(recipientId: RecipientId, type: Long, timestamp: Long, outgoing: Boolean): InsertResult {//*TM_SA*/ make fun open
  ...
  open fun updateCallLog(messageId: Long, type: Long) {//*TM_SA*/ make fun open
    writableDatabase
      .update(TABLE_NAME)
      .values(
        TYPE to type,
  ...
  open fun updateGroupCall(//*TM_SA*/ make fun open
    messageId: Long,
    eraId: String,
    joinedUuids: Collection<UUID>,
    isCallFull: Boolean
  ...
  open fun insertEditMessageInbox(mediaMessage: IncomingMessage, targetMessage: MmsMessageRecord): Optional<InsertResult> {//*TM_SA*/ make fun open
  ...
  private fun getThreadIdFor(retrieved: IncomingMessage): ThreadTable.ThreadIdResult {
	...
	// TM_SA make protected
  protected fun rawQueryWithAttachments(where: String, arguments: Array<String>?, reverse: Boolean = false, limit: Long = 0): Cursor {
  ...
  open fun markAsOutbox(messageId: Long) {//*TM_SA*/ make fun open
    val threadId = getThreadIdForMessage(messageId)
  ...
  open fun markAsSending(messageId: Long) {//*TM_SA*/ make fun open
    val threadId = getThreadIdForMessage(messageId)
  ...
  open fun markAsSentFailed(messageId: Long) {//*TM_SA*/ make fun open
    val threadId = getThreadIdForMessage(messageId)
  ...
  open fun markAsSent(messageId: Long, secure: Boolean) {//*TM_SA*/ make fun open
    val threadId = getThreadIdForMessage(messageId)
  ...
  protected open fun markAsRemoteDeleteInternal(messageId: Long) {//*TM_SA*/ make fun protected open
  ...
  open fun markDownloadState(messageId: Long, state: Long) {//*TM_SA*/ make fun open
    writableDatabase
      .update(TABLE_NAME)
  ...
  protected open fun setMessagesRead(where: String, arguments: Array<String>?): List<MarkedMessageInfo> {//*TM_SA*/ make fun protected open
  ...
  open fun insertMessageInbox(//*TM_SA*/ make fun open
  ...
  @Throws(MmsException::class)
  open fun insertMessageOutbox(//*TM_SA*/ make fun open
    message: OutgoingMessage,
    threadId: Long,
    forceSms: Boolean,
  ...
  protected open fun deleteMessage(messageId: Long, threadId: Long = getThreadIdForMessage(messageId), notify: Boolean = true, updateThread: Boolean = true): Boolean {//*TM_SA*/ make fun protected open
  ...
  protected open fun incrementReceiptCountInternal(targetTimestamp: Long, receiptAuthor: RecipientId, receiptSentTimestamp: Long, receiptType: ReceiptType, messageQualifier: MessageQualifier, stopwatch: Stopwatch? = null): Set<MessageReceiptUpdate> {//*TM_SA*/ make fun protected open
